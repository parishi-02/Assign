...
<html>
<head>
    <title>Update Assessment Scores</title>
...
<body>
    ...
    <c:set var="username" value="${pageContext.request.userPrincipal.name}" />
        ...
        <c:when test="${username == 'admin'}">
            <c:set var="formAction" value="${pageContext.request.contextPath}/admin/saveAssessmentScore" />
        </c:when>
        <c:otherwise>
        ...
        <div class="table-container">
            <form method="post" action="${formAction}">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2>Update Assignment Scores</h2>
                    <div>
                        <button type="submit" class="btn btn-primary">Update All Trainee Marks</button>
                    ...
                    <tbody>
                        <c:forEach items="${trainees}" var="trainee">
                            <tr>
                                ...
                                <c:forEach items="${assessmentPlanDto.marksObtained}" var="parameter">
                                   <c:if test="${trainee.employeeId eq parameter.key}">
                                        <td>
                                            ${parameter.value}
                                        </td>
                                        <td>
                                            <button type="button" class="btn btn-info assign-score" data-id="${trainee.employeeId}">Update Score</button>
                                            <div class="action-buttons" data-id="${trainee.employeeId}">
                                                ...
                                                <input type="hidden" name="marks[${trainee.employeeId}]" value="${parameter.value}">
                                                <input type="hidden" name="assessmentId" value="${assessmentPlanDto.assessmentId}">
                                            </div>
                                        </td>
   ...
   $(document).ready(function() {
          ...
          var marksValue = "";
            ...
            $(document).on('click', '.save-score', function() {
                ...
                if(marks>${assessmentPlanDto.totalScore})
                {
                    var message = 'Marks should be less than or equal to Total score (' + ${assessmentPlanDto.totalScore} + ')';
