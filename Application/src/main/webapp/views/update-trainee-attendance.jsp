...
<body>
    ...
    <c:set var="username" value="${pageContext.request.userPrincipal.name}" />
        ...
        <c:when test="${username == 'admin'}">
            <c:set var="formAction" value="${pageContext.request.contextPath}/admin/updateTraineeAttendance" />
        </c:when>
        <c:otherwise>
            <c:set var="formAction" value="${pageContext.request.contextPath}/trainer/updateTraineeAttendance" />
        </c:otherwise>
        </c:choose>
    <div class="container">
        <div class="table-container">
            <form action="${formAction}" method="POST" modelAttribute="trainerDto">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2 class="mb-0">Trainee Update List</h2>
                    <div>
                ...
                <hr>
                <input type="hidden" name="attendanceDate" value="${attendanceDto.attendanceDate}"/>
                    ...
                    <tbody>
                        <c:forEach items="${traineeList}" var="parameter">
                            <tr>
                                ...
                                <td>
                                    <c:forEach items="${attendanceStatus}" var="value">
                                        <c:if test="${fn:contains(value, parameter.employeeId)}">
                                           <c:set var="found" value="true" scope="page"/>
                                           <c:choose>
                                               <c:when test="${fn:contains(value, 'halfDay')}">
                                                   <button type="button" class="btn btn-success mark-present"
                                                           data-id="${parameter.employeeId}"
                                                           data-state="absent">Present</button>
                                                   <button type="button" class="btn btn-danger mark-absent"
                                                           data-id="${parameter.employeeId}"
                                                           data-state="present">Absent</button>
                                                   <button type="button" class="btn btn-secondary mark-halfDay"
                                                           data-id="${parameter.employeeId}"
                                                           data-state="halfDay">Half Day</button>
                                                   <input type="hidden" name="traineeStates[${parameter.employeeId}]" class="trainee-state" value="halfDay">
                                               </c:when>
                                               <c:when test="${fn:contains(value, 'present')}">
                                                   <button type="button" class="btn btn-secondary mark-present"
                                                           data-id="${parameter.employeeId}"
                                                           data-state="absent">Present</button>
                                                   <button type="button" class="btn btn-danger mark-absent"
                                                           data-id="${parameter.employeeId}"
                                                           data-state="present">Absent</button>
                                                   <button type="button" class="btn btn-primary mark-halfDay"
                                                           data-id="${parameter.employeeId}"
                                                           data-state="halfDay">Half Day</button>
                                                   <input type="hidden" name="traineeStates[${parameter.employeeId}]" class="trainee-state" value="present">
                                               </c:when>
                                               <c:when test="${fn:contains(value, 'absent')}">
                                                   <button type="button" class="btn btn-success mark-present"
                                                           data-id="${parameter.employeeId}"
                                                           data-state="absent">Present</button>
                                                   <button type="button" class="btn btn-secondary mark-absent"
                                                           data-id="${parameter.employeeId}"
                                                           data-state="present">Absent</button>
                                                   <button type="button" class="btn btn-primary mark-halfDay"
                                                           data-id="${parameter.employeeId}"
                                                           data-state="halfDay">Half Day</button>
                                                   <input type="hidden" name="traineeStates[${parameter.employeeId}]" class="trainee-state" value="absent">
                                               </c:when>
                                               <c:when test="${fn:contains(value, 'false')}">
                                                  <button type="button" class="btn btn-success mark-present"
                                                          data-id="${parameter.employeeId}"
                                                          data-state="absent">Present</button>
                                                  <button type="button" class="btn btn-danger mark-absent"
                                    ...
                                    <input type = "hidden" name="attendanceId" value = "${attendanceDto.attendanceId}"/>
                                </td>
                            </tr>
                        </c:forEach>
                    </tbody>
