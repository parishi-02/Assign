...
<body>
  ...
  <c:set var="username" value="${pageContext.request.userPrincipal.name}" />
      ...
      <c:when test="${username == 'admin'}">
          <c:set var="formAction" value="${pageContext.request.contextPath}/admin/saveUpdateAssessment" />
      </c:when>
      <c:otherwise>
          <c:set var="formAction" value="${pageContext.request.contextPath}/trainer/saveUpdateAssessment " />
      </c:otherwise>
      </c:choose>
        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card shadow">
                    ...
                    <form action="${formAction}" id="updateAssessment" method="POST">
                        <div class="card-body" id="innerContainer">
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <h2 class="mb-0">Update Assessment</h2>
                                <div>
                            ...
                            <div class="form-group">
                                <label for="subTopic">Session Description:</label>
                                <input type="text" class="form-control" id="subTopic" name="subTopic" value="${assessmentDto.subTopic}" readonly />
                            </div>
                            <input type="hidden" name="assessmentId" value="${assessmentDto.assessmentId}" aria-hidden="true"/>

                            <div class="form-group">
                                <label for="plannedDate">Planned Date</label>
                                <input type="date" class="form-control" id="plannedDate" name="plannedDate" required value="${assessmentDto.plannedDate}" readonly/>
                            </div>
                            <div class="form-group">
                                <label for="totalScore">Total marks:</label>
                                <input type="number" class="form-control" id="totalScore" name="totalScore" value="${assessmentDto.totalScore}" />
                            </div>
                            <div class="form-group">
                                <label for="actualDate">Enter Actual Date:</label>
                                <input type="date" class="form-control" id="actualDate" name="actualDate" value="${assessmentDto.actualDate}"/>
                            </div>
                            <div class="form-group">
                                <label for="remarks">Enter Remarks:</label>
                                <input type="text" class="form-control" id="remarks" name="remarks" placeholder="Enter remarks" value="${assessmentDto.remarks}"/>
                            </div>
                        </div>
                    </form>
                </div>
  ...
  <!-- Modal for error messages -->
     ...
     <script>
         document.addEventListener('DOMContentLoaded', function() {
             ...
             var form = document.getElementById('updateAssessment');
             var cancelButton = document.getElementById('cancelButton');
             var loading = document.getElementById('loading');
             var errorMessage = document.getElementById('errorMessage');
             var defaultTotalScore = totalScoreInput.value;

             function updateFieldStates() {
                 var totalScore = totalScoreInput.value.trim();
                 ...
                 if (totalScore !== '' && totalScore !== '${assessmentDto.totalScore}') {
             ...
             totalScoreInput.addEventListener('input', updateFieldStates);
             ...
             form.addEventListener('submit', function(event) {
                 if (!validateForm()) {
                     ...
                     // Check if totalScore is empty, and set it to the default value if needed
                     if (totalScoreInput.value.trim() === '') {
                         totalScoreInput.value = defaultTotalScore;
